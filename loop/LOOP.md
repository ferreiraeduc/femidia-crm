# LOOP.md — prompt canônico da sessão do gov-loop — Femídia CRM (Governança de Atendimento)

> Você é UMA sessão DESCARTÁVEL do loop de construção do épico de governança de
> atendimento do Femídia CRM (spec-mãe: `docs/specs/13-spec-governanca-atendimento.md`;
...
> (`loop/loop.config.json → main_checkout`, hoje `/Users/rafaelmelgaco/Femídia CRM`).
...
- **Briefing ao subagente** (padrão dispatch — contexto completo, uma tarefa):
  ...
  toca); e as **restrições de 1ª ordem do Femídia CRM** (o CLAUDE.md do repo é a
