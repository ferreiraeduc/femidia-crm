...
**Onde o loop RODA: no checkout principal — `/Users/rafaelmelgaco/Femídia CRM`
(`loop/loop.config.json → main_checkout`) — DEPOIS que a branch `gov/setup` for
mergeada em `main`.** O worktree `Femídia CRM-gov` existe só para montar a
...
- `plan/features.json` populado (26 features G1-01..G6-04); `plan/progress.md`
  ...
  ```json
  {
    ...
    "main_checkout": "/Users/rafaelmelgaco/Femídia CRM",
...
```bash
cd /Users/rafaelmelgaco/Femídia CRM && claude
> /deskcomm-gov-loop
```
...
#!/usr/bin/env bash
set -u
REPO="/Users/rafaelmelgaco/Femídia CRM"; cd "$REPO" || exit 1
...
`/bin/bash -lc '/Users/rafaelmelgaco/Femídia CRM/loop/run-session.sh'` e
