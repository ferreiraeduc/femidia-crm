...
cat > "$PROJ/.env" <<ENV
APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:latest
...
check ".env aponta para a imagem da versão instalada" grep -q '^APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:1.1.0$' .env
...
check "o worker é pinado na MESMA versão do app" \
  grep -q '^WORKER_IMAGE=ghcr.io/ferreiraeduc/femidia-worker:1.1.0$' .env
check "o scheduler é pinado na MESMA versão do app" \
  grep -q '^SCHEDULER_IMAGE=ghcr.io/ferreiraeduc/femidia-scheduler:1.1.0$' .env
...
check "a imagem do .env continua intacta" grep -q '^APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:latest$' .env
...
check "NÃO reescreveu a imagem do .env" grep -q '^APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:latest$' .env
...
pin_caso "app pinado + worker/scheduler AUSENTES → acusa os dois" \
  "APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:1.3.0" "worker scheduler"
pin_caso "app pinado + worker em canal móvel → acusa" \
  "APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:1.3.0
WORKER_IMAGE=ghcr.io/ferreiraeduc/femidia-worker:stable
SCHEDULER_IMAGE=ghcr.io/ferreiraeduc/femidia-scheduler:1.3.0" "worker"
pin_caso "as três na mesma versão → silêncio" \
  "APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:1.3.0
WORKER_IMAGE=ghcr.io/ferreiraeduc/femidia-worker:1.3.0
SCHEDULER_IMAGE=ghcr.io/ferreiraeduc/femidia-scheduler:1.3.0" ""
...
case "$*" in
  *"Config.Image"*)  printf 'ghcr.io/ferreiraeduc/femidia-worker:stable
' ;;
...
R="$(autopin "APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:1.3.0")"
check "chave AUSENTE → preenche os dois" test "$R" = "worker scheduler"
check "  e grava a versão da imagem em execução, não um canal" \
  grep -q "^WORKER_IMAGE=ghcr.io/ferreiraeduc/femidia-worker:1.3.0$" "$PIN_DIR/.env"
...
R="$(autopin "APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:1.3.0
WORKER_IMAGE=ghcr.io/ferreiraeduc/femidia-worker:stable
SCHEDULER_IMAGE=ghcr.io/ferreiraeduc/femidia-scheduler:stable")"
check "canal móvel EXPLÍCITO → não toca (é decisão de quem opera)" test -z "$R"
check "  o :stable escolhido continua lá, intacto" \
  grep -q "^WORKER_IMAGE=ghcr.io/ferreiraeduc/femidia-worker:stable$" "$PIN_DIR/.env"
...
R="$( printf 'APP_IMAGE=ghcr.io/ferreiraeduc/femidia-crm:1.3.0\n' > "$PIN_DIR/.env"
