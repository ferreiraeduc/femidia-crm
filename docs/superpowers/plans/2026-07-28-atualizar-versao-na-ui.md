...
**Goal:** O dono de uma instalação self-host atualiza o Femídia CRM clicando num botão na própria tela, sem abrir terminal nem SSH.
...
- **Worktree:** `~/Femídia CRM-update`, branch `feat/atualizar-pela-ui` (já criada a partir de `origin/main`). Não trabalhe no clone principal — ele está sujo com trabalho de outra sessão.
...
const COMANDO_MANUAL = "cd Femídia CRM && bash hostgator-setup-kit/update.sh";
...
export APP_IMAGE="ghcr.io/ferreiraeduc/femidia-crm:${TARGET_TAG#v}"
