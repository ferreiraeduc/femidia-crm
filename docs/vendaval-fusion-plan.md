# Fusão Vendaval → Femídia CRM — briefing de execução (Terminal B)
...
**Um repositório só: o Femídia CRM.** Fusão total do back-end do Vendaval (bom, mas nunca ligado à
realidade) dentro do Femídia CRM (Next.js + TS + Supabase + WAHA + AI SDK — CRM real com chat ao vivo,
...
Femídia CRM como borda — o contrato exato está em `~/vendaval/docs/specs/edge-contract.md` (referencia
arquivos e linhas reais do Femídia). Como agora é UM repo, a maquinaria de "dois sistemas" colapsa em
chamadas internas.

**Encaixes verificados no código real do Femídia:**
...
- Entrada: WAHA → `event_log` (RPC `emit_event`). O Femídia tem um dispatcher nativo (`dispatchAgents`).
...
- Modelo de dados do Femídia é o CANÔNICO: `organizations`/`contacts`/`conversations`/`messages`/
...
→ resposta aparece no CHAT do Femídia. *Prova:* você manda um WhatsApp de verdade a um número conectado e VÊ o
...
**Fase 2 — continuidade + config real.** Memória/RAG/follow-up no turno (usando histórico real do Femídia) +
...
- Do Femídia: siga o `CLAUDE.md`/`AGENTS.md`/MANIFEST do repo (migrations, RLS, LGPD nativa, multi-tenant).
